...
function Header() {
    ...
    const model = (e) => {
      ...
      if (search.style.visibility === "hidden" ) search.style.visibility = "visible";
    ...
    useEffect( () => {
        ...
        (
            async () => {
                ...
                // if(window.innerWidth>850) me
                const res = await axios.get(`http://localhost:8085/category/read/`);
    ...
    return (
        <>  
            <div className="day" style={{ width: "20%" }}></div>
            <div className="nutmenu">
                <svg  id="hien" onClick={hienmenu} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-border-style hien nutmenuclick" viewBox="0 0 16 16">
                ...
                <svg  id="an" onClick={anmenu} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-backspace-reverse-fill an nutmenuclick" viewBox="0 0 16 16">
            ...
            <div className="icon" style={ {height: "50px"} }>
                ...
                <Link to="/cart">
                    <div className="so" id="sl" style={{ position:'absolute', marginLeft: '9%', backgroundColor: 'black', color: 'white', borderRadius: '50%',  width: '20px', lineHeight: '20px',  textAlign: 'center',}}>{state.cartItems.length}</div>
            ...
            <div className="menu">
                ...
                <Link to="product/productlist">DANH SÁCH SẢN PHẨM</Link>
                {
                    (cate.length>0) && (
                        cate.map((item,index)=>{
                            return (
                                <Link to={`product/category/${item._id}`} key={index}>{item.categoryName}</Link>
                            )
                        })
                    )
                 }
                ...
                <Link href="#">ĐỒ ĐÔI</Link>

                <Link href="#">PHỤ KIỆN</Link>

                <Link href="#">CHÍNH SÁCH</Link>

                <Link href="#">TOTO FANZONE</Link> */}
                {/* Danh mục */}
            </div>
            <div className="hotline">
                <img src="/images/hotlinehd.png" alt="" />
                <Link href="#">1900 633 501</Link>
            </div>
            <div className="nen-tang">
                <Link>
